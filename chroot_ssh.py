...
# Copyright 2023 Denys Razumov <zxzharmlesszxz@gmail.com>
...
DOCUMENTATION = '''
    name: chroot_ssh
    short_description: connect to remote chroot via SSH client binary
    description:
        - This connection plugin allows Ansible to communicate to the target machines in remote chroots through normal SSH command line.
    ...
    options:
      ...
      chroot_path:
        description:
            The chroot path at remote host to connect to.
        env:
            - name: CHROOT_PATH
        ini:
            - key: chroot_path
              section: chroot_ssh_connection
        vars:
          - name: chroot_path
        cli:
          - name: chroot_path
...
class Connection(ConnectionBase):
    """ ssh+chroot connection """

    transport = 'chroot_ssh'
    has_pipelining = True
    ...
    def _connect(self):
        ...
        self.chroot_path = pipes.quote(self.get_option('chroot_path'))
    ...
    def _file_transport_command(self, in_path, out_path, action):
        ...
        if action == 'get':
            ...
            if sys.version_info[0] >= 3:
                ...
                with open(to_bytes(out_path, errors='surrogate_or_strict'), 'w') as out_file:
                    out_file.write(stdout)
        else:
        ...
        if returncode == 0:
            ...
            display.warning(u'Transfer mechanism failed on %s. Use ANSIBLE_DEBUG=1 to see detailed information' % (host))
            display.debug(u'%s' % to_text(stdout))
            display.debug(u'%s' % to_text(stderr))

        if returncode == 255:
            raise AnsibleConnectionFailure("Failed to connect to the host: %s" % (to_native(stderr)))
        else:
    ...
    def exec_command(self, cmd, in_data=None, sudoable=True):
        ...
        chroot_cmd = u'sudo chroot %s /bin/sh -c %s'
        cmd = chroot_cmd % (pipes.quote(self.chroot_path), pipes.quote(cmd))
